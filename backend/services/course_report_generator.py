...
class CourseReportGenerator:
    ...
    def _generate_student_summaries(
        ...
        for student_id in student_ids:
            ...
            sessions_count = (
                self.db.query(SessionDB)
                .filter(
                    SessionDB.student_id == student_id,
                    SessionDB.start_time >= period_start,
                    SessionDB.start_time <= period_end,
                )
                .count()
            ...
            competency = (
                latest_eval.overall_competency_level if latest_eval else "N/A"
            )
            ...
            # Critical risks (DB stores lowercase values)
            ...
            summaries.append(
                {
                    ...
                    "ai_dependency": round(float(avg_ai_dependency), 3),
    ...
    def _categorize_students_by_risk(
        ...
        categorized = {"critical": [], "high": [], "medium": [], "low": [], "none": []}

        for student_id in student_ids:
            # Get highest risk level for student (DB stores lowercase)
            highest_risk = (
                self.db.query(RiskDB)
                .filter(
                    RiskDB.student_id == student_id,
            ...
            if highest_risk:
                categorized[highest_risk.risk_level].append(student_id)
            else:
                categorized["none"].append(student_id)
