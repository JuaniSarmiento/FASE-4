...
@router.get(
    ...
    description="Compara los caminos cognitivos de diferentes estudiantes en la misma actividad (HU-DOC-003)"
)
async def compare_students(
    activity_id: str = Query(..., description="ID de la actividad"),
    student_ids: List[str] = Query(None, description="IDs específicos de estudiantes (opcional)"),
...
@router.get(
    ...
    description="Obtiene alertas de estudiantes con dificultades o riesgos críticos (HU-DOC-004)"
)
async def get_teacher_alerts(
    ...
    **Ejemplo:**
    ...
    for session in active_sessions:
        ...
        duration_hours = (datetime.utcnow() - session.start_time).total_seconds() / 3600.0
...
@router.post(
    ...
    description="Marca una alerta como atendida por el docente"
)
