...
export const TraceabilityViewer: React.FC = () => {
  ...
  const loadInteractions = async (sessionId: string) => {
    ...
    try {
      const response = await httpClient.get(`/sessions/${sessionId}/interactions`);
      setInteractions((response as any).interactions || []);
    } catch (error: any) {
      showToast(`Error cargando interacciones: ${error.message}`, 'error');
      setInteractions([]);
    }
  };
  ...
  const loadTrace = async (interactionId: string) => {
    ...
    try {
      ...
      showToast(`Error cargando traza: ${error.message}`, 'error');
      setTrace(null);
    } finally {
      setLoading(false);
    }
              ...
              {interactions.map((interaction, idx) => (
                ...
                >
                  ...
                  <div className="interaction-preview">
                    {interaction.user_input.substring(0, 50)}...
                  </div>
                </li>
              ))}
            </ul>
